...
class Invoice(Base, table=True):
    ...
    invoice_number: str = Field(sa_column=Column(
        String(255), nullable=False, unique=True
    )) #
    store_name: str = Field(sa_column=Column(
        String(255), nullable=False
    )) #
    store_address: str = Field(sa_column=Column(
        String(255), nullable=False
    )) #
    vat_number: str = Field(sa_column=Column(
        String(255), nullable=False
    )) #
    date: datetime = Field(sa_column=Column(
        pg.TIMESTAMP(timezone=True), nullable=False
    )) #
    total: Decimal = Field(sa_column=Column(
        pg.NUMERIC(10, 2),
        default=Decimal("0.00"),
        nullable=False
    )) #
    taxes: Decimal = Field(sa_column=Column(
        pg.NUMERIC(10, 2),
        default=Decimal("0.00"),
        nullable=False
    )) #
    seller_taxes: Decimal = Field(sa_column=Column(
        pg.NUMERIC(10, 2),
        default=Decimal("0.00"),
        nullable=False
    )) #
    net_total: Decimal = Field(sa_column=Column(
        pg.NUMERIC(10, 2),
        default=Decimal("0.00"),
        nullable=False
    )) #
    user_name: str = Field(sa_column=Column(
        String(255),
        nullable=False
    )) #
    account_id: str = Field(sa_column=Column(
        String(255),
        nullable=False
    )) #
    status: InvoiceStatus = Field(
        sa_column=Column(
            pg.ENUM(
                InvoiceStatus,
                name="invoice_status_enum",  # consistent type name
                create_type=True,
            ),
            nullable=False,
        )
